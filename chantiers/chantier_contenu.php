...
if (!$entrepriseId) { http_response_code(403); exit('Entreprise non définie.'); }
...
/* ========= Droits d’accès =========
   - administrateur : OK
   - chef : OK s’il est assigné au chantier
*/
$allowed = false;
if ($role === 'administrateur') {
  $allowed = true;
} elseif ($role === 'chef') {
  // retirez AND entreprise_id = :eid si la table ne possède pas cette colonne
...
/* ========= Listes (pour la modale de transfert) ========= */
$stAllChantiers = $pdo->prepare("
  SELECT id, nom
  FROM chantiers
  WHERE entreprise_id = ?
  ORDER BY nom
");
$stAllChantiers->execute([$entrepriseId]);
$allChantiers = $stAllChantiers->fetchAll(PDO::FETCH_KEY_PAIR);

$stAllDepots = $pdo->prepare("
  SELECT id, nom
  FROM depots
  WHERE entreprise_id = ?
  ORDER BY nom
");
$stAllDepots->execute([$entrepriseId]);
$allDepots = $stAllDepots->fetchAll(PDO::FETCH_KEY_PAIR);

/* ========= Données stock de CE chantier =========
   Si stock_chantiers n’a pas entreprise_id, on sécurise via le JOIN chantiers c.
*/
$sql = "
  SELECT
    s.id             AS article_id,
...
/* ========= Catégories & sous-catégories présentes ========= */
$stCats = $pdo->prepare("
...
");
$stCats->execute([':cid' => $chantierId, ':eid' => $entrepriseId]);
$categories = $stCats->fetchAll(PDO::FETCH_COLUMN);
...
");
$stSubs->execute([':cid' => $chantierId, ':eid' => $entrepriseId]);
...
<div class="container mt-4">
  ...
  <!-- Retour Menu chantier -->
  <div class="d-flex justify-content-between align-items-center mb-3">
    <a href="javascript:history.back()" class="btn btn-outline-secondary">← Retour</a>
    <h1 class="mb-0">Stock du chantier : <?= htmlspecialchars($chantier['nom']) ?></h1>
    <div></div>
  </div>
...
<script>
  ...
  window.subCategories = <?= json_encode($subCategoriesGrouped, JSON_UNESCAPED_UNICODE) ?>;
</script>
