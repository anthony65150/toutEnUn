...
// Récupérer l’ID du dépôt associé à l'utilisateur
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['transfert_id']) && $depotId) {
    ...
    $stmt = $pdo->prepare("
        ...
        AND destination_type = 'depot' AND destination_id = ?
    ");
    $stmt->execute([$transfertId, $depotId]);
    $transfert = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$transfert) {
        $_SESSION['error_message'] = "Transfert non trouvé ou non autorisé.";
        header("Location: stock_depot.php");
        exit;
    }
    ...
    try {
        ...
        // Si source = dépôt → on avait déjà décrémenté → on remet
        ...
        $message = "❌ Le dépôt #$depotId a refusé le transfert de {$transfert['quantite']} x article #{$transfert['article_id']}.";
